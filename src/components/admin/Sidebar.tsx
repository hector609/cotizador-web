...
/**
 * Sidebar — sidebar fijo izquierdo del rediseño "REVENTAR mode"
 * (dark-glassmorphism premium tipo Linear/Vercel).
 *
 * Promovido desde `app/dashboard/_sidebar.tsx` a `components/admin/` para
 * que `/dashboard/historial`, `/dashboard/chat`, `/dashboard/clientes`,
 * etc. lo puedan importar conforme migren al nuevo shell oscuro.
 *
 * Estrategia de transición: cada página que migra al shell dark importa
 * este componente como wrapper de su contenido. NO se mete en
 * `layout.tsx` porque algunas rutas (`/dashboard/historial`, etc.) aún
 * usan la `DashboardNav` legacy blanca y los dos shells se solaparían.
 *
 * Responsive:
 *  - Desktop (lg+): sidebar fijo w-64 a la izquierda.
 *  - Mobile/tablet (< lg): se muestra mobile top-bar + drawer toggle.
 *
 * Activo: el caller pasa `active` como key.
 */
...
interface SidebarProps {
  active: SidebarKey;
  /** Identificador del DAT (vendedor_id, email, alias) para el cluster inferior. */
  userLabel?: string;
  /** Subtítulo del cluster (rol, tenant, etc.). */
  userSubtitle?: string;
  ...
  return (
    ...
    >
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M3 12a9 9 0 109-9M3 12V6m0 6h6"
      />
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
