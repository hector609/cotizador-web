...
/**
 * /api/onboarding/complete — proxy POST al backend.
 * Marca el onboarding como completado y dispara email de bienvenida.
 */
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error("[api/onboarding/complete POST] sign error", e);
    return errJson("Estamos realizando tareas de mantenimiento. Intenta en unos minutos.", 500);
  }

  let upstream: Response;
  try {
    upstream = await fetch(`${BOT_API_URL}/api/v1/onboarding/complete`, {
      method: "POST",
      headers: {
        ...authHeader,
    ...
    console.error("[api/onboarding/complete POST] backend fetch error", e);
    return errJson("No pudimos completar el onboarding. Reintenta.", 502);
  }
  ...
  try {
    ...
    console.error("[api/onboarding/complete POST] json parse", e);
