...
function CotizarPageInner() {
  ...
  // Modo activo. Acepta `?mode=excel|experto|wizard` para deep-link desde
  // el chat UI (página `/dashboard/cotizar`).
  const [mode, setMode] = useState<Mode>(() => {
    const m = params.get("mode");
    return m === "excel" || m === "experto" ? m : "wizard";
  });
      ...
      <header className="bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-4 text-sm">
            ...
            <Link href="/dashboard/cotizar-old" className="text-blue-700 font-medium">
              Wizard
            </Link>
            <Link
